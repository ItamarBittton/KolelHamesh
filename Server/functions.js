...
const copyDates = (req, res, next) => {
    ...
    // sql.q(`select t1.student_id 
    //         from ${process.env.database}.tb_student t1 
    //         where t1.colel_id = ${req.currentUser.colel_id} and t1.id_deleted = 0`, function(results) {
    //     const student_list = results.results;
    ...
    sql.q(`INSERT INTO ${process.env.database}.tb_daily (student_id, date, presence) 
                                                  (select t1.student_id, DATE_ADD(t1.date, INTERVAL DATEDIFF('${pasteEndDate}', '${copyEndDate}') DAY), t1.presence 
