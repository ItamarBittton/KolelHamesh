...
const getStatics = (req, res) => {
    let { staticsType, sortType, dateType, amountOfHistory } = req.params;
    const colel_id = staticsType == 1 ? staticsType : 1;
    let query = `SELECT sum(*) AS 'oneTimeStudents'
                 FROM ${process.env.database}.tb_one_time_students t1,
                      ${process.env.database}.tb_student t2
                 WHERE t1.student_id = t2.id and
                       t2.colel_id in (SELECT t3.id
                                       FROM ${process.env.database}.tb_colel t3
                                       WHERE t3.id = ${colel_id} or 1 = ${colel_id}`
};
