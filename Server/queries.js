...
function getStats(req) {
    ...
    return `SELECT (SELECT SUM(amount) FROM tb_onetime_student WHERE ${colelId}) AS 'extraStudents',
                   (SELECT COUNT(*) FROM tb_student WHERE ${colelId} and is_deleted = 0) AS 'students',
                   (SELECT COUNT(*) FROM tb_score t1
		                       JOIN tb_student t2 ON (t1.student_id = t2.id)
    ...
    return `(select t7.id as "colel_id",
                ...
                from tb_student t1 
                                    left outer join (select t2.student_id, count(t2.student_id) as 'present'
                                        from tb_daily t2
                                        where t2.presence >= 0 and month(t2.date) = ${month} and year(t2.date) = ${year}
                                    ...
                                    left outer join (select t2.student_id, count(t2.student_id) as 'missed'
                                        from tb_daily t2
                                        where t2.presence = -1 and month(t2.date) = ${month} and year(t2.date) = ${year}
                                        group by t2.student_id) t4 on (t1.id = t4.student_id)

                                    left outer join (select t2.student_id, count(t2.student_id) as 'appMissed'
                                        from tb_daily t2
                                        where t2.presence = -2 and month(t2.date) = ${month} and year(t2.date) = ${year}
...
const getStatics = (dateTypeStr, startDate, endDate, colel_id) => {
    return [
        `SELECT ${dateTypeStr} as 'date', t2.name as 'colel_id', SUM(t1.amount) as 'data'
        ...
        `SELECT MAKEDATE(t1.year, t1.month) as 'date', t2.name as 'colel_id', count(t1.oral_score) as 'data'
                ...
                WHERE t3.colel_id = t2.id and
                      t3.id = t1.student_id and
                      t1.oral_score > 0 and
                    t1.year between year('${startDate}') and year('${endDate}') and
                    t1.month between month('${startDate}') and month('${endDate}')
        ...
        `SELECT ${dateTypeStr} as 'date', t2.name as 'colel_id', (count(t1.presence) * t4.lesson_last) / 60 as 'data'
                FROM ${process.env.database}.tb_daily t1,
                    ${process.env.database}.tb_colel t2,
                    ${process.env.database}.tb_student t3,
                    ${process.env.database}.tbk_settings t4
                WHERE t3.colel_id = t2.id and
                      t3.id = t1.student_id and
