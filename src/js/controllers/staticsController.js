angular.module('RDash')
    .controller('staticsController', function($scope, Data) {
        ...
        $scope.renderChart = function(chartType) {
            ...
            var currentData = $scope.oneTimeStudents[0];

            for (i = 0; i < $scope.oneTimeStudents.length; i += j) {
                dataPoints = [];
                for (j = 0; currentData.colel_id === currentColelId; j++) {
                    dataPoints.push({ x: new Date(currentData.date), y: currentData.data });
                    // currentColelId = $scope.oneTimeStudents[i + j].colel_id;
                    currentData = $scope.oneTimeStudents[i + j];
            ...
            // for (i = 0; i < $scope.oneTimeStudents.length; i++) {
            //     var currentData = $scope.oneTimeStudents[i];
            //     dataPoints.push({ x: new Date(currentData.date), y: currentData.data });

            //     if (currentColelId === currentData.colel_id) {} else {
            //         data.push({
            //             type: chartType.val,
            //             showInLegend: true,
            //             name: currentColelId + "",
